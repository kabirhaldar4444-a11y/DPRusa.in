...
try {
  ...
  fs.writeFileSync('scratch/temp_news.cjs', jsContent);
  const result = execSync('node scratch/temp_news.cjs', { encoding: 'utf-8' });
  ...
  if (fs.existsSync('scratch/temp_news.cjs')) {
    fs.unlinkSync('scratch/temp_news.cjs');
  }
}
